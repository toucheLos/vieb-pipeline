...
| 36 | **External review reconciled** | **done** | review §5 steps 1, 2, 4 already run: island context replication `FAIL` (review assumed a positive control), freeze-tip branch reached, grooming refused for a cause the review lacks — keypoint-driven registration injects apparent motion | `results/REVIEW.md` |
| 37 | **STABILISE — keypoint-free crop registration** | **done — NOT_A_RESULT** | B and P refused: the median-background mask fails on 213 of 300 recordings (71%) because the animal is in its own background. Incumbent measured without selection: 100% of 23,910 duplicate-frame values read as motion; jitter → head energy +0.340 [+0.285, +0.392]. Gate 4 unattainable by construction (D23). §7 caught an OpenCV in-place write, fixed before any gate was read | `results/STABILISE.md`, `stabilise.json`, D22, D23 |
| 38 | **STABILISE 2 — keypoint-masked background** | **done — NOT_A_RESULT** | mask now mouse-sized (0.311 × bl², was 0.151) but 298 of 300 recordings refused: 7.5% of pixels at the median recording never seen without the resting animal's box; mask-area rule alone would refuse 53%. Gate 3 ill-conditioned, gate 4 calibration did not transfer (D24). Median-background route closed; a segmenter is owed | `results/STABILISE2.md`, `stabilise2.json`, D24 |
| 39 | **STABILISE 3 — SAM ViT-B mask track** | **done — NOT_A_RESULT** | SB/SP refused on 144 of 300 recordings; gate 0 FAILS for all arms (disc on the animal 66.9% [58.5, 75.0] vs ≥ 90%): SAM, prompted by its own last box, **drifts onto the arena wall** with IoU ≈ 0.95 and stays there (overlay published). Rebuilt gates 3 and 4 reproduce the incumbent calibration (K 3.54×, 3.93×). Gate 2's premise questioned (D25) | `results/STABILISE3.md`, `stabilise3.json`, `results/stabilise3/`, D25 |
| 40 | **STABILISE 4 — SAM prompted from the located animal; SP only** | **done — SP FAIL (gate 4)** | drift gone: disc on animal 93.7%, refused 56/300. SP passes gates 0–3: exact on duplicates (0/22,950), **planted 2 px jitter moves it 1.000× vs incumbent 11.8×**, still frames 1.04× perfect. Fails moving plants at 17×: ECC settles between the static bar floor and the mouse (leaves 67–82% of the in-disc difference; an earlier "8.8%" figure is withdrawn, D27) | `results/STABILISE4.md`, `stabilise4.json` |
| 41 | **STABILISE 5 — ECC on the animal only (SM) + keypoint-agreement gate** | **done — SM FAIL, SP FAIL (opposite gates)** | SM follows moving animals: moving plant 1.31× perfect (SP 17×); on real fast frames its recovered body motion is bracketed 0.83–1.27 (SP 0.14–0.82). But SM breaks still frames: 0.6% of duplicate pairs non-zero, 2.3× the identity on immobile frames (SP 1.04×). Likely SAM-centroid init; owed: identity init or per-pair selection | `results/STABILISE5.md`, `stabilise5.json`, D26, D27 |
| 42 | **STABILISE 6 — masked ECC from the better of two starts (SM6) + chained keypoint gate** | **done — SM6 FAIL (gates 3, 4)** | identity start fixes duplicates (0/22,932); jitter-immune (1.001×); chained keypoint agreement PASS (β 0.95–f 1.07; SP 0.17–0.56). Still-frame gate unchanged (2.28): not the start — the 0.6 bl disc reaches the bar floor, which SM6's 0.07 px animal fits move (ratio 1.75/2.28/2.84 at 0.4/0.6/0.8 bl; SP flat). Moving plant 1.305. Owed: score gates on the animal's own pixels | `results/STABILISE6.md`, `stabilise6.json` |
| 43 | **STABILISE 7 — gates on the animal's own pixels** | **done — SM6 FAIL (gate 4 only); registration line STOPPED by its pre-set rule** | floor reading confirmed: SM6 still-frame ratio 1.028 on animal pixels (2.279 with floor). SM6 passes gates 0, 1, 2, 3, 6; fails the moving plant 2.13 [1.93, 2.35] vs bar 1.60 (perfect-pose K0 1.43). Conclusion: keypoint-free alignment not solved for moving heads; exact for still ones. A still-window grooming stage would be a separate, investigator-decided registration | `results/STABILISE7.md`, `stabilise7.json`, `stabilise7_calibration.json` |
| 44 | **SAM mask vs DLC keypoints — agreement** | **done (descriptive)** | agree on 81.0% [77.7, 84.1] of SAM keyframes. When not, SAM has usually clipped the head: nose/ears outside the mask on 13–16% of all keyframes (54–64% of disagreeing), centre 0.5%. The self-check attribution ("DLC suspect" 7.1% vs "SAM suspect" 0.5%) is not trustworthy — example frames show the mask, not DLC, at fault. Next: keypoints as SAM point prompts | `results/AGREEMENT.md`, `agreement.json`, `agreement_examples.jpg` |
| 45 | **SAM point prompts (plain)** | **done (descriptive) — not adopted** | ears (unprompted) outside the mask 14.3% → 7.0%, but a wrong DLC keypoint drags SAM onto the arena wall: masks > 1.5× box on 13.2% of context-B frames vs 1.9% in A | `results/POINT_PROMPTS.md`, `point_prompts.json` |
| 46 | **SAM point prompts (guarded: each detector checks the other)** | **done (descriptive) — adopt for the mask track** | ears outside 14.5% → 10.3% (−4.3 pts [3.1, 5.5]); no inflation (p95 area +4%). Gain mostly in A (10.0 → 5.9%); B 20.1 → 15.8%. **Head coverage is arena-dependent at baseline (B clips the head 2× as often): head-region context comparisons must condition on per-frame head coverage** | `results/POINT_PROMPTS_GUARDED.md`, `point_prompts_guarded.json` |
