"""STABILISE 5: ECC on the animal only (SM), and agreement with the keypoints on real motion.

    sbatch jobs/stabilise5.slurm                 # 12 GPU shards
    python3 scripts/stabilise5.py --phase combine

READ results/STABILISE5_PREREGISTRATION.md FIRST, with Amendment 1 (D26).
It changes one thing in STABILISE 4 and adds one gate, so this script loads
`stabilise4.py` (and through it STABILISE 3) and reuses gates 0, 1, 3, 4 and 5:

* §1 + Amendment 1: arm SM, `scan_track(..., mask_fn=...)` with SAM's mask
  carried to t-1 and t and eroded 0.04 body lengths. SP is carried unchanged.
* §2: gate 6, f = sum|d_arm|^2 / sum(d_kp . d_arm) on real fast frames.
"""
...
def work_dir() -> str:
    return os.path.join(config.REPO, "work", "stabilise5")
...
def shard(a) -> int:
    ...
    for n, r in enumerate(mine, 1):
        ...
        np.savez_compressed(
            ...
            W_SP=s["P|W"], W_SM=s["M|W"], center=center, center_ok=center_ok,
            **got, **plants, **jit)
        f6 = {arm: _f_one({"W": s[f"{src}|W"], "center": center,
                           "center_ok": center_ok, "speed": speed})
              for src, arm in (("P", "SP"), ("M", "SM"))}
        log(f"  {n}/{len(mine)} {rid}: refused {nan_frac:.3f}, on-animal "
            f"{inside}/{scored}, gate6 f SP {f6['SP'][0]:.3f} SM {f6['SM'][0]:.3f}"
            f" ({f6['SM'][1]} pairs), jitter SM "
            f"{jit.get('jit__SM__jit', np.nan) / max(1e-12, jit.get('jit__SM__orig', np.nan)):.3f}, "
...
def _f_one(z) -> tuple[float, int]:
    num, den, k = _pairs(z["W"], np.asarray(z["center"]),
                         np.asarray(z["center_ok"], dtype=bool),
                         np.asarray(z["speed"]))
    return (num / den if den else float("nan")), k


def _gate6(recs, meta, arm, refused) -> Read:
    obj = {"dataset": "luna", "arm": f"stabilise5|{arm}|gate6_keypoint_agreement"}
    ...
    if len(per) < st.MIN_ANIMALS:
        return Read("NOT_A_RESULT", obj,
                    ...
                    f"against {st.MIN_ANIMALS}", n_effective=max(1, len(per)))
    b = boot.animal_interval(list(per.values()), list(per), how="mean",
                             n_boot=st.N_BOOT, seed=st.SEED)
    ok = float(b["lo"]) >= G6_BAND[0] and float(b["hi"]) <= G6_BAND[1]
    return Read("PASS" if ok else "FAIL", obj,
                (f"on real fast frames the alignment recovers "
                 f"{b['point']:.3f} [{b['lo']:.3f}, {b['hi']:.3f}] of the "
                 f"keypoint-measured body motion over {len(per)} animals; the "
...
def _gate2(recs, meta, refused) -> dict[str, Read]:
    ...
    out["K"] = Read("NOT_A_RESULT", obj,
                    ...
                    n_effective=len(ku), detail={"interval": kb,
                                                 "powered": powered})
    for arm in ("SP", "SM"):
        obj = {"dataset": "luna", "arm": f"stabilise5|{arm}|gate2_jitter"}
...
def combine(a) -> int:
    ...
    out = a.out or config.PATHS.result("stabilise5.json")
    reads: dict = {}
    kc = st._k_check(recs)
    reads["k_check"] = kc
    if not kc["ok"]:
        log(f"  §7 STOP: K is not the incumbent: {kc}")
        write_json({"stage": "stabilise5", "stopped": "§7", "k_check": kc}, out)
        return 1
    ...
    for arm in ARMS:
        ...
        obj = {"dataset": "luna", "arm": f"stabilise5|{arm}"}
        if arm == "K":
        ...
        log(f"  {arm}: {v} -- {why}")
    write_json({**provenance.header(anchors.LUNA, stage="stabilise5",
                                    unverified="a registration instrument"),
                "inherited_digest": spine.digest(),
                "registration": "results/STABILISE5_PREREGISTRATION.md",
                "deviation": "DEVIATIONS.md D26",
