"""STABILISE 7: the same arms, with gates 2-5 scored on the animal's own pixels.

    sbatch jobs/stabilise7.slurm                 # 12 GPU shards
    python3 scripts/stabilise7.py --phase combine

READ results/STABILISE7_PREREGISTRATION.md FIRST, and
results/stabilise7_calibration.json, which set gates 3 and 4's bars from the
incumbent alone. This file is `stabilise6.py` edited only where §1-§3 say:

* full-disc series keep their usual keys (so §7 and gate 1 are unchanged);
  animal-pixel series (disc intersected with SAM's eroded mask) end ``__a``,
  and gates 2-5 are computed on a view that reads them;
* gate 3 in [0.80, 1.25]; gate 4 in [0.80, 1.60]; gate 6 unchanged;
* SAM's accepted keyframe masks are saved, bit-packed.
"""
...
def work_dir() -> str:
    return os.path.join(config.REPO, "work", "stabilise7")
    ...
    for i in still:
        ...
        for run, pw in (("orig", p0), ("jit", pj)):
            ...
            s = rg.scan_track(lambda: iter(grey), pw, tr, radii_px=[r],
                              ...
                              region_fn=_mfn(tr, bl))
            got[run] = {arm: np.asarray(s[f"{src}|head|{r}|a"])
...
def shard(a) -> int:
    ...
    for n, r in enumerate(mine, 1):
        ...
        s = rg.scan_track(st._open(video), pose, tr,
                          ...
                          best_start=True, region_fn=_mfn(tr, bl))
        got = {f"{RENAME[arm]}__{reg}__{st._rkey(x)}": s[f"{arm}|{reg}|{bl * x}"]
               for arm in RENAME for reg, _ in rg.REGIONS for x in st.RADII_BL}
        got.update({f"{RENAME[arm]}__{reg}__{st._rkey(x)}__a":
                    s[f"{arm}|{reg}|{bl * x}|a"]
                    for arm in RENAME for reg, _ in rg.REGIONS
...
def _packed_masks(tr: dict) -> dict:
    """§3: every accepted keyframe's mask, bit-packed, with its box."""
    ts, boxes, bits = [], [], []
    for t, (x0, y0, crop) in sorted(tr["key_crops"].items()):
        ts.append(t)
        boxes.append((x0, y0, crop.shape[0], crop.shape[1]))
        bits.append(np.packbits(crop.ravel()))
    lens = np.asarray([b.size for b in bits], dtype=np.int64)
    return {"mask_t": np.asarray(ts, dtype=np.int64),
            "mask_box": np.asarray(boxes, dtype=np.int64).reshape(-1, 4),
            "mask_offsets": np.concatenate([[0], np.cumsum(lens)]),
            "mask_bits": (np.concatenate(bits) if bits
                          else np.zeros(0, dtype=np.uint8))}


def _animal_view(z: dict) -> dict:
    """The same record with every animal-pixel series under its plain key."""
    v = dict(z)
    for k in z:
        if k.endswith("__a"):
            v[k[:-3]] = z[k]
...
def _banded(read_fn, band, recs, meta, arm, refused) -> Read:
    """Re-read a STABILISE 3 band gate against this stage's band."""
    saved = s3.BAND
    try:
        s3.BAND = band
        return read_fn(recs, meta, arm, refused)
    finally:
        s3.BAND = saved
...
def _gate6(recs, meta, arm, refused) -> Read:
    obj = {"dataset": "luna", "arm": f"stabilise7|{arm}|gate6_chained"}
...
def _gate2(recs, meta, refused) -> dict[str, Read]:
    ...
    obj = {"dataset": "luna", "arm": "stabilise7|K|gate2_jitter_control"}
    ...
    out["K"] = Read("NOT_A_RESULT", obj,
                    ...
                    n_effective=len(ku), detail={"interval": kb,
                                                 "powered": powered})
    for arm in ("SP", "SM6"):
        obj = {"dataset": "luna", "arm": f"stabilise7|{arm}|gate2_jitter"}
...
def combine(a) -> int:
    ...
    out = a.out or config.PATHS.result("stabilise7.json")
    reads: dict = {}
    kc = st._k_check(recs)
    reads["k_check"] = kc
    if not kc["ok"]:
        log(f"  §7 STOP: K is not the incumbent: {kc}")
        write_json({"stage": "stabilise7", "stopped": "§7", "k_check": kc}, out)
        return 1
    log(f"  §7 K reproduces the incumbent: max |diff| {kc['max_abs_diff']:.2e}")
    ref = {arm: s3._refused3(recs, arm) for arm in ARMS}
    recs_a = {rid: _animal_view(z) for rid, z in recs.items()}
    g2 = _gate2(recs_a, meta, ref)
    for arm in ARMS:
        ...
        reads[arm] = {"detail": det,
                      ...
                      "gate3": _banded(s3._gate3, G3_BAND, recs_a, meta, arm,
                                       ref[arm]).to_dict(),
                      "gate4": _banded(s3._gate4, G4_BAND, recs_a, meta, arm,
                                       ref[arm]).to_dict(),
                      "gate5_curve": s2._gate5(recs_a, arm, ref[arm], fps),
    ...
    for arm in ARMS:
        ...
        obj = {"dataset": "luna", "arm": f"stabilise7|{arm}"}
        if arm == "K":
        ...
        log(f"  {arm}: {v} -- {why}")
    write_json({**provenance.header(anchors.LUNA, stage="stabilise7",
                                    unverified="a registration instrument"),
                "inherited_digest": spine.digest(),
                "registration": "results/STABILISE7_PREREGISTRATION.md",
                "calibration": "results/stabilise7_calibration.json",
